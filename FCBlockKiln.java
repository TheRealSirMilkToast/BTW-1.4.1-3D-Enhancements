...
public class FCBlockKiln extends Block
{
    ...
    private Icon[] m_unfiredCookIcons;
    private Icon[] m_foodIcons;
    ...
    public void registerIcons( IconRegister register )
    {
        ...
        //<DK ModPack>
        m_unfiredCookIcons = new Icon[7];

        for ( int iTempIndex = 0; iTempIndex < 7; iTempIndex++ )
        {
        	m_unfiredCookIcons[iTempIndex] = register.registerIcon( "fcOverlayUnfiredBrick_" + ( iTempIndex + 1 ) );
        }
        
        m_foodIcons = new Icon[7];

        for ( int iTempIndex = 0; iTempIndex < 7; iTempIndex++ )
        {
        	m_foodIcons[iTempIndex] = register.registerIcon( "fcOverlayFood_" + ( iTempIndex + 1 ) );
        }
        //</DK ModPack>
    }
	...
	//i, j, k are the KILN block coords
	public Icon GetCookTextureForCurrentState( IBlockAccess blockAccess, int i, int j, int k )
	{
		if(blockAccess.getBlockId(i, j + 1, k) == FCBetterThanWolves.fcUnfiredPottery.blockID && blockAccess.getBlockMetadata(i, j + 1, k) == FCBlockUnfiredPottery.m_iSubtypeUncookedCake)
		{
			return null;
		}
		else if(blockAccess.getBlockId(i, j + 1, k) == FCBetterThanWolves.fcUnfiredPottery.blockID && (blockAccess.getBlockMetadata(i, j + 1, k) == FCBlockUnfiredPottery.m_iSubtypeUncookedCookies || blockAccess.getBlockMetadata(i, j + 1, k) == FCBlockUnfiredPottery.m_iSubtypeUncookedCookiesIAligned))
		{
			return null;
		}
		else if(blockAccess.getBlockId(i, j + 1, k) == FCBetterThanWolves.fcUnfiredPottery.blockID && blockAccess.getBlockMetadata(i, j + 1, k) == FCBlockUnfiredPottery.m_iSubtypeUncookedPumpkinPie)
		{
			return null;
		}
		else if(blockAccess.getBlockId(i, j + 1, k) == FCBetterThanWolves.fcUnfiredPottery.blockID && (blockAccess.getBlockMetadata(i, j + 1, k) == FCBlockUnfiredPottery.m_iSubtypeUncookedBread || blockAccess.getBlockMetadata(i, j + 1, k) == FCBlockUnfiredPottery.m_iSubtypeUncookedBreadIAligned))
		{
			return null;
		}
		else if(blockAccess.getBlockId(i, j + 1, k) == FCBetterThanWolves.fcUnfiredPottery.blockID && blockAccess.getBlockMetadata(i, j + 1, k) != FCBlockUnfiredPottery.m_iSubtypeNetherBrick && blockAccess.getBlockMetadata(i, j + 1, k) != FCBlockUnfiredPottery.m_iSubtypeNetherBrickIAligned)
		{
			int iTextureIndex = ( GetCookCounter( blockAccess, i, j, k ) / 2 ) - 1;
			
			if ( iTextureIndex >= 0 && iTextureIndex <= 6 )
			{
				return m_unfiredCookIcons[iTextureIndex];
			}
		}
		else
		{
